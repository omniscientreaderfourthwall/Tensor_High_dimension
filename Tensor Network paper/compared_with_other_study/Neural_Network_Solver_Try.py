...
def fit(loss_func_1, loss_func_2, optimizer, Para):
    ...
    Opt_State_1 = optimizer.init(Para)
    Opt_State_2 = optimizer.init(Para)

    @jit
    def step_1(param, opt_state, batch_1):

        loss_value, grads = jax.value_and_grad(loss_func_1)(param, batch_1)
    ...
    @jit
    def step_2(param, opt_state, batch_2, dens_ref_points):

        loss_value, grads = jax.value_and_grad(loss_func_2)(param, batch_2, dens_ref_points)
        updates, opt_state = optimizer.update(grads, opt_state, param)
        param = optax.apply_updates(param, updates)
    ...
    for epoch in range(args.epochs):
        ...
        loss_value, Para, Opt_State_1 = step_1(Para, Opt_State_1, batch_1)
        loss_value_ref, Para, Opt_State_2 = step_2(Para, Opt_State_2, batch_ref, batch_dens_ref)
...
Schedule = optax.polynomial_schedule(init_value = 3e-3, end_value=1e-5, power = 2, transition_steps = 70000, transition_begin=10000)
opt = optax.chain(optax.adam(learning_rate= Schedule) )
