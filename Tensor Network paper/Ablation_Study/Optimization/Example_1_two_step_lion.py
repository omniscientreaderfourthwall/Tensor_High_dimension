...
parser.add_argument('--test_case', type=str, default="Exp 4.1 two step lion")
...
parser.add_argument('--r_sde', type = float, default = 2.1467)
...
Center_O = jnp.array([-0.0056,  0.0026])
...
func_into = ( lambda data:  -1 * jnp.asarray([ 8 * data[0] * (data[0]**2 + data[1]**2 - 1),
                                                  8 * data[1] * (data[0]**2 + data[1]**2 - 1) ] )
                                                 )

div_into = ( lambda data:  -1 * 16 * (2 * data[0]**2 + 2 * data[1]**2 - 1) )
...
Loss function
'''
Loss_Params = {"reg_bound": 1.0,"reg_diff": 1.0, "penal_param":5e4}



def Loss_Func(param_kernel, param_coeff, data_batch):

    param = param_kernel|param_coeff
...
Path_Name = args.test_case + "_" + "_" + bw + "_" + kn + "/"
...
def fit(loss_func, optimizer, resampler, Para):
    ...
    Opt_State_kernel = optimizer.init(Para_kernel)
    Opt_State_coeff = optimizer.init(Para_coeff)

    @jit
    def step_kernel(param_kernel, param_coeff, opt_state, batch):
        loss_value, grads = jax.value_and_grad(loss_func, argnums = 0)(param_kernel, param_coeff, batch)
        updates, opt_state = optimizer.update(grads, opt_state, param_kernel)
        param_kernel = optax.apply_updates(param_kernel, updates)
        return loss_value, param_kernel, opt_state

    @jit
    def step_coeff(param_kernel, param_coeff, opt_state, batch):
        loss_value, grads = jax.value_and_grad(loss_func, argnums = 1)(param_kernel, param_coeff, batch)
        updates, opt_state = optimizer.update(grads, opt_state, param_coeff)
        param_coeff = optax.apply_updates(param_coeff, updates)
        return loss_value, param_coeff, opt_state
    ...
    for epoch in tqdm.tqdm(range(args.epochs)):
        ...
        if (epoch // 100) % 2 == 0:
           loss_value, Para_kernel, Opt_State_kernel = step_kernel(Para_kernel, Para_coeff, Opt_State_kernel, Dataset)
        
        elif (epoch // 100) % 2 == 1:
           loss_value, Para_coeff, Opt_State_coeff = step_coeff(Para_kernel, Para_coeff, Opt_State_coeff, Dataset)
...
Schedule = optax.polynomial_schedule(init_value = 9e-4, end_value=8e-6, power = 2, transition_steps = 140000, transition_begin=20000)
