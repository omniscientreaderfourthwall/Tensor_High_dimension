...
parser.add_argument('--test_case', type=str, default="Exp 4.5")
...
parser.add_argument('--r_sde', type = float, default = 2.8842168)
...
Center_O = jnp.array([-0.01091017, 0.01022847, -0.00244608, 0.00682626, 0.00188168, 0.00056796, 0.00050733, -0.00071843, -0.00536333, -0.00065273])



def Accurate_Func_10dm(data):
    inside_exp = ( -2.5 * ( (data[:3]**2).sum() + 0.1 * (data[0]*data[1] + data[0] * data[2] + data[1] * data[2]) )
                -2.0 * ( ( data[3:6]**2).sum() + 0.2 *(data[3]*data[4] + data[3]* data[5] + data[4] * data[5]) )
                -3.0 * ( (data[6:8]**2).sum() - 0.01 * data[6] * data[7]  )
                -3.0 * ( (data[8:]**2).sum() -0.01 * data[8] * data[9]     )
                )

    result = (2 * data[8]**2 + 0.02) * jnp.exp(inside_exp) * (1/1.4138) * (1/1.9964) * (1/1.0472) * (1/0.3700)
    return result

vec_dens_accurate = vmap(Accurate_Func_10dm, 0, 0)

#Function which helps the computation of loss functional
func_into = ( lambda data: (- 1 * jnp.asarray([5 * data[0] +   0.25 * (data[1] + data[2]),
                                  5 * data[1] + 0.25 * (data[0] + data[2]),
                                  5 * data[2] + 0.25 * (data[0] + data[1]),

                                  4 * data[3] + 0.4 * (data[4] + data[5]) ,
                                  4 * data[4] + 0.4 * (data[3] + data[5]) ,
                                  4 * data[5] + 0.4 * (data[3] + data[4]) ,

                                   6 * data[6] -0.03 * data[7],
                                   6 * data[7] - 0.03 * data[6],

                                   6 * data[8] -0.03 * data[9] - (2 * data[8])/(data[8]**2 + 0.01),
                                   6 * data[9] - 0.03 * data[8]
                                ]
                           ) ) )

div_into = ( lambda data: (-1.0 * (5.0 +
                       5.0 +
                       5.0 +
                       4.0  +
                       4.0  +
                       ...
                       6.0  +
                       6.0  +
                       6.0  + 2 *(data[8]**2 - 0.01)/(data[8]**2 + 0.01)**2 +
                       6.0
...
r = jnp.array([2.291168, 2.1985602, 2.1700895, 2.436545, 2.6220152, 2.383542, 2.2343159,
 1.8750496, 2.1955984, 2.0016267])
...
args_init = [key_ini, 0.9 * min_r, jnp.sqrt(min_r), min_r, Shape]
...
wendland_vec = jit(vmap(wendland_1_test, in_axes = (None, 0, None)))

def wendland_integral(h, mesh, shift):
    delta_t = mesh[1] - mesh[0]
    batch_size = 1000
    num_batches = mesh.shape[0] // batch_size + (mesh.shape[0] % batch_size != 0)
    batches = jnp.array_split(mesh, num_batches)
    func_mesh = wendland_vec(h, batches[0], shift)
    for batch in batches[1:]:
        func_mesh = jnp.concatenate((func_mesh, wendland_vec(h, batch, shift)) )
    results = 0.5 * delta_t * (2 *(func_mesh).sum(axis = 0) - func_mesh[0] - func_mesh[-1])
    return results
...
def combine_k_integral(bandwidth, dist_pa, mesh, shift):

    result = []

    for i in range(dim):

        dim_result = ( dist_pa[0][:,:,[i]] * k1[0](bandwidth[0][:,:,[i]], mesh[i], shift[0][:,:,[i]]) +
                  dist_pa[1][:,:,[i]] * k2[0](bandwidth[1][:,:,[i]], mesh[i], shift[1][:,:,[i]]) +
                  dist_pa[2][:,:,[i]] * k3[0](bandwidth[2][:,:,[i]], mesh[i], shift[2][:,:,[i]]) )
    ...
    final_result = jnp.concatenate(result, axis = -1)
    return final_result
...
def KDE_integral(param, mesh):
    ...
    output = (     ( alpha_2 * combine_k_integral(param["width"], 
                                                alpha_1,
                                                mesh, 
                                                param["shifts"])  ).sum(axis = 0)
                                                  ).prod(axis = -1)
    result = (coeff * output).sum()
...
Loss function
'''
Loss_Params = {"reg_bound": 100.0,"reg_diff": 1.0, "penal_param":1e2}
...
Schedule = optax.polynomial_schedule(init_value = 3e-4, end_value=5e-6, power = 2, transition_steps = 70000, transition_begin=10000)
...
radius_search = [0.3, 0.6, 0.9, 1.2, 1.5, 1.8, 2.1, 2.4]

for radius in radius_search:
    mesh = []
    for j in range(dim):
        mesh_j = (jnp.linspace(-radius, radius, endpoint = True, num = 20001)).reshape(1, -1)
        mesh.append(mesh_j)
    
    mesh = jnp.concatenate(mesh, axis = 0)
    mesh_set.append(mesh)

radius_result = []

for mesh in mesh_set:
    radius_result.append(KDE_integral(Final_Param, mesh))


print(radius_result)

Integral_file_name = Save_Path + "Integral_List.txt"

with open(Integral_file_name, 'w') as file:
     file.write(str(radius_result) + str(radius_search))
