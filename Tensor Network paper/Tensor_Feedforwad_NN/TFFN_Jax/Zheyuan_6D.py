...
parser.add_argument('--cutoff', type=float, default=1.6, help="cutoff of the domain")
...
parser.add_argument('--dtype', type=str, default="float64", help="dtype, float64 is significantly better than float32")
...
norm_sol = estimate_integral_rosenbrock() if "Rosenbrock" in args.dataset else jnp.ones(1)
test_u = test_u - jnp.log(norm_sol) * (args.dim // 2) # normalize the log-likelihood
...
class PINN_TNN:
    def __init__(self):
        ...
        self.x = x
        self.u = u.reshape(-1)
        self.quad_x, self.quad_w = quad_x.reshape(-1, 1), quad_w.reshape(-1)
    ...
    def Resample(self): # sample random points at the begining of each iteration
        self.xf = np.random.uniform(low=-args.cutoff, high=args.cutoff, size=(args.N_f, args.dim))
        return
