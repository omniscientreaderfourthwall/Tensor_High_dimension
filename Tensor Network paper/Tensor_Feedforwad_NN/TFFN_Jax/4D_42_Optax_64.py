...
parser.add_argument('--dataset', type=str, default="FourDCase")
parser.add_argument('--epochs', type=int, default=100000, help="Adam training epochs")
parser.add_argument('--dim', type=int, default=4, help="problem dimensionality")
...
parser.add_argument('--cutoff', type=float, default=2.6472, help="cutoff of the domain: [-cutoff, cutoff]^dim")
parser.add_argument('--K', type=int, default=64, help="Number of separable subnets")
parser.add_argument('--net_scal', type=float, default= 1.0, help="network_scaling")
parser.add_argument('--accmu_step', type=int, default=1, help="accumulation steps")
parser.add_argument('--dtype', type=str, default="float32", help="dtype, float 32 recommended")
...
O_center = np.array([-0.0043, 0.1048, 0.0044, 0.0081])

if args.dtype == "float64": 
    jax.config.update("jax_enable_x64", True)

c = 0.3; c_1 = 0.0; c_2 = 0.1; c_3 = 1.0
#Function of F
F = lambda x, y: 3 * ( (x**4 - y)**2 + 2 * y**2 )
#gradient of F
F_nabla = lambda x, y: 3 * jnp.asarray([8 * ((x**4) - y) * (x**3), -2 * ((x**4) - y) + 4 * y])
# function g
g = lambda x, y: ( x**2 - c * y * x + y**2)
#gradient of g
g_nabla = lambda x, y: jnp.asarray([2 * x - c * y, 2 * y - c * x])
#Function of G
G = lambda x, y: g(x, y) + g(x-c_1, y-c_1)
#gradient of capital G
G_nabla = lambda x, y: g_nabla(x, y) + g_nabla(x-c_1, y-c_1)
#diffusion variation FD
V = lambda x, y: c_2 * (x**2) * (y**2)
#gradient of FD
V_nabla = lambda x, y: c_2 * jnp.asarray([2 * x * (y**2), 2 * (x**2) * y])

#True PDF
def dens_est_nk1(data):
    H = F(data[:, 0], data[:, 1]) + G(data[:, 2], data[:, 3])
    result = jnp.exp(- H)
    return result
def load_data_FourDCase(d):
    args.output_dim = 1
    N_test = args.N_test
    test_x = np.random.uniform(low=-1, high=1, size=(N_test, d))
    test_u = dens_est_nk1(test_x)
    return test_x, test_u

test_x, test_u = load_data_FourDCase(d=args.dim)
...
norm_sol = np.array([(1.02044) * 1.58877])
test_u = test_u / norm_sol.item()

def data_filter_high_probability(my_array, Accurate, high_probability):
    result = my_array[Accurate > high_probability]
    return result

print("Maximal value of the PDF: ", np.max(test_u))
x = data_filter_high_probability(test_x, test_u, 0.01)
u = dens_est_nk1(x) / norm_sol.item()
print("Num of test points with prob > 0.01 / 0.05 / 0.1: ", len(np.where(u > 0.01)[0]), len(np.where(u > 0.05)[0]), len(np.where(u > 0.1)[0]))

quad_x, quad_w = composite_quadrature_1d(args.N_quad // 16, -args.cutoff, args.cutoff, 16)  # This quadrature is for estimating the integral of the model
...
class PINN_TNN:
    def __init__(self):
        ...
        init_value = args.lr
        Schedule = optax.polynomial_schedule(init_value = init_value, end_value=8e-6, power = 2, transition_steps = int(args.epochs * 0.8/args.accmu_step), transition_begin=0)
        ...
        self.predit_residual = jax.vmap(self.FourDCase, (None, 0))

        self.loss = self.get_loss_pinn_multi

    def F(self, x, y): return 3 * ( (x**4 - y)**2 + 2 * y**2 )
    def nabla_F(self, x, y): return 3 * jnp.asarray([8 * ((x**4) - y) * (x**3), -2 * ((x**4) - y) + 4 * y])
    
    def g(self, x, y): return ( x**2 - c * y * x + y**2)
    def nabla_g(self, x, y): return jnp.asarray([2 * x - c * y, 2 * y - c * x])

    def G(self, x, y): return g(x, y) + g(x - c_1, y - c_1)
    def nabla_G(self, x, y): return  g_nabla(x, y) + g_nabla(x-c_1, y-c_1)
    
    def V(self, x, y): return c_2 * (x**2) * (y**2)
    def nabla_V(self, x, y): return c_2 * jnp.asarray([2 * x * (y**2), 2 * (x**2) * y])
    
    def H(self, x): return self.F(x[0], x[1]) + self.G(x[2], x[3])
    
    def fk_nk1(self, params, data):
        func = - jnp.asarray(
            [ F_nabla(data[0], data[1])[0],
            F_nabla(data[0], data[1])[1],
            (V(data[2], data[3]) + c_3) * G_nabla(data[2], data[3])[0] + V(data[2], data[3])*G_nabla(data[2], data[3])[1]
            - V_nabla(data[2], data[3]).sum(),
            (V(data[2], data[3]) + c_3) * G_nabla(data[2], data[3])[1] + V(data[2], data[3])*G_nabla(data[2], data[3])[0]
            - V_nabla(data[2], data[3]).sum()
            ]
        )
        result = func * self.neural_net(params, data)
        return result

    def func_jac_nk1(self, params, data):
        return jax.jacfwd(self.fk_nk1, argnums = 1)(params, data)
    
    def fk_nk1_div(self, params, data):
        jacob_matrix = self.func_jac_nk1(params, data)
        jaco_iden = jnp.identity( jacob_matrix.shape[0])
        return (jacob_matrix * jaco_iden).sum()
    
    def fk_nk1_hess(self, params, data):
        Hessian_M = jax.jacfwd(jax.jacrev(self.neural_net, argnums=1), argnums=1)(params, data)
        # Hessian_M = hessian(dens_est_nk1, argnums = 1)(params, data)
        diag_Hessian_M = jnp.diagonal(Hessian_M)

        new_func_nk1 = lambda params, data: V(data[2], data[3]) * self.neural_net(params, data)
        Hessian_sys = jax.jacfwd(jax.jacrev(new_func_nk1, argnums=1), argnums=1)(params, data)
        result = 2 * ( diag_Hessian_M[0] + diag_Hessian_M [1]
                    + c_3 * (diag_Hessian_M[2] + diag_Hessian_M [3])
                    + 2 * Hessian_sys[2][3] + Hessian_sys[2][2] + Hessian_sys[3][3] )
        return result
    
    def neural_net(self, params, x):
        #return jnp.exp(-self.H(x))
    ...
    def FourDCase(self, params, data): 
        return - self.fk_nk1_div(params, data) + 0.5 * self.fk_nk1_hess(params, data)
    ...
    def train(self):
        ...
        Save_Path = Save_Path = '/content/drive/MyDrive/Review_Experiments/Zheyuan/'
